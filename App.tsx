...
function App() {
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>) => {
    ...
    if (id === 'website-honeypot') {
      ...
      // Strict Sanitization: Digits only, max 10 chars
      const sanitized = value.replace(/\D/g, '').slice(0, 10);
      setFormData(prev => ({ ...prev, [id]: sanitized }));
      
      // Clear error if valid length reached
      if (sanitized.length === 10) setPhoneError('');
    } else {
  ...
  const handlePhoneBlur = () => {
    if (formData.phone.length > 0 && formData.phone.length < 10) {
      setPhoneError('Ο αριθμός πρέπει να αποτελείται από 10 ψηφία.');
    } else {
      setPhoneError('');
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // 3. Construct Payload
    // Map automation type to Greek labels
    const automationTypeLabels: Record<string, string> = {
      'appointments': 'Ραντεβού & κρατήσεις',
      'leads': 'Διαχείριση leads & follow-up',
      'crm': 'Email / CRM αυτοματοποίηση',
      'support': 'Εξυπηρέτηση πελατών (chat / WhatsApp)',
      'internal': 'Εσωτερικές διαδικασίες & reports',
      'unsure': 'Δεν είμαι σίγουρος — θέλω προτάσεις'
    };

    const automationTypeLabel = automationTypeLabels[formData.interest] || formData.interest;

    // Format timestamp for Greece timezone DD/MM/YYYY HH:mm
    const now = new Date();
    const greekDate = new Date(now.toLocaleString('en-US', { timeZone: 'Europe/Athens' }));
    const dd = String(greekDate.getDate()).padStart(2, '0');
    const mm = String(greekDate.getMonth() + 1).padStart(2, '0');
    const yyyy = greekDate.getFullYear();
    const hh = String(greekDate.getHours()).padStart(2, '0');
    const min = String(greekDate.getMinutes()).padStart(2, '0');
    const greekTimestamp = `${dd}/${mm}/${yyyy} ${hh}:${min}`;

    const payload = {
      fullName: formData.name,
      email: formData.email,
      phone: formData.phone || '',
      automationType: formData.interest,
      automationTypeLabel: automationTypeLabel,
      message: formData.message,
      pageUrl: window.location.href,
      timestamp: now.toISOString(),
      timestampGreek: greekTimestamp
    };

    try {
      const response = await fetch('https://n8n.srv1392648.hstgr.cloud/webhook/contact', {
        method: 'POST',
        headers: {
      ...
      setFeedbackMessage('Το μήνυμα στάλθηκε επιτυχώς!');
      ...
      setSubmitStatus('error');
      setFeedbackMessage('Παρουσιάστηκε σφάλμα. Προσπαθήστε ξανά.');
    }
  };
      ...
      <main className="relative z-10">
        ...
        <section id="contact" className="py-16 md:py-24 relative">
          <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div className="glass rounded-2xl p-6 sm:p-8 md:p-10 shadow-2xl hover:!bg-slate-900/60 hover:!border-white/10">
              <form className="space-y-6" onSubmit={handleSubmit}>
                ...
                <div>
                   ...
                   <input 
                    ...
                    required
                    className={`w-full px-4 py-3 rounded-lg border bg-slate-900/50 text-white text-base focus:outline-none focus:ring-2 focus:ring-[#5227FF] focus:border-transparent transition-all placeholder:text-slate-600 appearance-none ${
                      phoneError ? 'border-red-500/50 focus:ring-red-500' : 'border-slate-700'
                    }`}
                  />
                  {phoneError && (
                    <p className="text-red-400 text-xs mt-2 font-medium animate-fade-in">
                      {phoneError}
                    </p>
                  )}
                </div>
                ...
                <div className="pt-4 flex flex-col items-center">
                  <button 
                    ...
                    className={`w-full sm:w-auto bg-[#5227FF] text-white font-bold py-4 px-12 rounded-lg hover:bg-[#411edb] transition-all shadow-lg shadow-[#5227FF]/20 text-lg border border-white/10 flex items-center justify-center gap-2 ${
                  ...
                  >
                    {submitStatus === 'submitting' ? (
                      ...
                      'Ας το δούμε μαζί — χωρίς καμία δέσμευση'
                    )}
                  </button>
